...
2. "Exhaustive":
 ...
 3. "Welcome to COMP3506":
       Extensive testing with the tightest possible time
       and memory restrictions
       leaving no room for redundant operations.
       Every possible corner case will be assessed here as well.

There will be hidden tests in each category that will be published
only after the assignment deadline.
...
def dora(graph: Graph, start: int, symbol_sequence: str,
    ...
    for i in range(visited_order.get_size()):
        x = visited_order[i]
        temp.append(Entry(freqs[x], DLLNode(x)))
    PQ.ip_build(temp)
    while PQ.get_size() > 1:
...
def _in_rad(c1: Compound, c2: Compound):
    if sqrt((c1.get_coordinates()[0]-c2.get_coordinates()[0])**2+(
        c1.get_coordinates()[1]-c2.get_coordinates()[1])**2) \
              <= c1.get_radius():
        return True
    else:
        return False
